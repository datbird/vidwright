  ...
  const items = clips
    .map((clip) => {
      ...
      if (!track) {
        ...
        const fallbackId = `vidwright-orphan-${fallbackType}`
...
export function buildPremiereXml({
  ...
  const projectLabel = sanitizeName(projectName, 'Vidwright Project')
  ...
  return [
    ...
    `      </audio>`,
    `    </media>`,
    `    <description>Exported from Vidwright project: ${escapeXml(projectLabel)}</description>`,
    `  </sequence>`,
    `</xmeml>`,
    '',
  ].join('\n')
