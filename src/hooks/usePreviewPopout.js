...
const POPOUT_NAME = 'vidwright-preview-popout'
const BOUNDS_KEY = 'vidwright-preview-popout-bounds'
...
export default function usePreviewPopout({ getSourceElement, onTogglePlay }) {
  ...
  const open = useCallback(() => {
    ...
    const blitToken = (child.__vidwrightBlitToken || 0) + 1
    child.__vidwrightBlitToken = blitToken

    const doc = child.document
    doc.title = 'Vidwright Preview'
    ...
    const blit = () => {
      if (child.closed || child.__vidwrightBlitToken !== blitToken) return
